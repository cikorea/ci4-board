...
| 항목 | MySQL | SQLite |
|------|-------|--------|
| 동시 쓰기 (50명) | 문제 없음 | WAL 모드로 개선되나 한계 있음 |
| 설치/운영 | 별도 서버 필요 | 파일 기반, 운영 간단 |
| 마이그레이션 비용 | 없음 (현재) | 높음 (스키마 재작성) |
| CI4 지원 | 완전 지원 | 지원하나 기능 제한 |
| JSON 쿼리 | 지원 | 제한적 |
| 트랜잭션 | 완전 지원 | WAL 모드 필요 |
| FOREIGN KEY | 완전 지원 | 별도 활성화 필요 |

**SQLite 채택 비권장 이유:**
- 현재 29개 테이블 + 외래키 + 트랜잭션 구조가 MySQL에 최적화
- `serialize()` 형식의 권한 데이터 등 기존 설계 변경 불가피
- 50명 이하라도 게시판 특성상 동시 쓰기(댓글, 조회수) 빈번

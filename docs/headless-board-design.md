...
| Phase | 내용 | 상태 |
|-------|------|------|
| Phase 1 | JWT 인프라, API 라우팅, Admin DB 분리 | ✅ 완료 |
| Phase 2 | 자체 로그인 API (login/register/refresh/me/profile/withdraw) | ✅ 완료 |
| Phase 3 | 게시판/게시글/댓글/쪽지/파일/어드민 API | ✅ 완료 |
| Phase 4 | 소셜 로그인 (Google/네이버/카카오) | ✅ 완료 |
| Phase 5 | 프론트엔드 (FE 개발자 필요) | ⏳ 대기 |
| Phase 6 | CMS | ⏳ 대기 |
| Phase 7 | 안정화 (PHPStan ✅, Rate Limit ⏳, CSRF ⏳) | 🔄 진행 중 |
...
6. ✅ `Config/Filters.php` 필터 별칭 등록
7. ✅ Admin DB 분리 (`ci4_board_admin`, Admin 전용 모델 4개)

### Phase 2 — 인증 API ✅ 완료

1. ✅ `Api/V1/AuthController` — `login()`, `register()`, `logout()`, `refresh()`, `me()`, `updateProfile()`, `withdraw()`
2. ✅ `Api/V1/Admin/AuthController` — 관리자 로그인 (Admin JWT 발급)
3. ✅ `Common.php`의 `current_group_idx()` JWT payload 기반으로 수정

### Phase 3 — 게시판/서비스 API ✅ 완료

1. ✅ `Api/V1/BoardController` (목록, 상세)
2. ✅ `Api/V1/ArticleController` (CRUD + 페이지네이션)
3. ✅ `Api/V1/CommentController` (CRUD)
4. ✅ `Api/V1/FileController` (업로드, 다운로드, 삭제)
5. ✅ `Api/V1/MessageController` (쪽지)
6. ✅ `Api/V1/Admin/*` (게시판/회원/게시글/설정)

### Phase 4 — 소셜 로그인 ✅ 완료

1. ✅ `tb_users_social` 마이그레이션
2. ✅ Google OAuth2 (`GoogleOAuthService`)
3. ✅ 네이버 OAuth2
4. ✅ 카카오 OAuth2 (`KakaoOAuthService`)

### Phase 5 — 프론트엔드 (FE 개발자 필요)

1. [ ] Next.js 프로젝트 세팅 (`ci4-board-web`)
2. [ ] React + Vite 어드민 프로젝트 세팅 (`ci4-board-admin`)
3. [ ] next-auth 소셜 로그인 연동
4. [ ] CI4 API 연동

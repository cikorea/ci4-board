...
<div class="card board-card">
    <div class="card-header py-3">
        ...
        <?= esc($board['bbs_name'] ?? $board['bbs_id']) ?> — <?= lang('App.edit_post') ?>
    </div>
        ...
        <form action="/board/<?= esc($board['bbs_id']) ?>/edit/<?= $post['idx'] ?>" method="post" enctype="multipart/form-data">
            ...
            <div class="d-flex gap-2">
                <button type="submit" class="btn btn-primary px-4">
                    <i class="bi bi-check-lg me-1"></i><?= lang('App.save_changes') ?>
                </button>
                <a href="/board/<?= esc($board['bbs_id']) ?>/view/<?= $post['idx'] ?>"
                   class="btn btn-outline-secondary px-4"><?= lang('App.cancel') ?></a>
            </div>
        </form>
    </div>
</div>

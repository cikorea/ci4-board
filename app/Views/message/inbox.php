...
<div class="card board-card">
    <div class="card-header d-flex justify-content-between align-items-center py-3">
        <div class="d-flex gap-3">
            <span class="fw-bold"><i class="bi bi-inbox me-1 text-primary"></i>받은 쪽지함</span>
            <a href="/message/sent" class="text-muted text-decoration-none" style="font-size:.88rem">보낸 쪽지함</a>
        </div>
    ...
    <div class="card-body p-0">
        <?php if (empty($messages)): ?>
            <p class="text-center text-muted py-5 mb-0">받은 쪽지가 없습니다.</p>
        <?php else: ?>
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
                    <thead class="table-light">
                        <tr>
                            <th style="width:110px" class="ps-3">보낸 사람</th>
                            <th>내용</th>
                            <th class="text-center d-none d-sm-table-cell" style="width:130px">받은 시각</th>
                        ...
                        ?>
                            <tr <?= $isUnread ? 'class="table-primary bg-opacity-25"' : '' ?>>
                                <td class="ps-3 fw-semibold" style="font-size:.88rem">
                                    <?= esc_db($m['sender_nickname'] ?? '알 수 없음') ?>
                                </td>
                                <td>
